...
const Menu = () => {
    ...
    const getCocktailAt = (indexOffset) =>{
        return sliderLists[(currentIndex + indexOffset + totalDrinks) % totalDrinks]
    }
            ...
            <nav className="cocktail-tabs" aria-label="Cocktail Navigation">
                {
                    sliderLists.map((drink, index) => {
                        ...
                        return(
                            <button onClick={() => goToSlide(index)} key={drink.id} className={`${isActive ? 'text-white border-white' : 'text-white/50 border-white/50'}`}>
                                {drink.name}
                            </button>
                        )
                    })
